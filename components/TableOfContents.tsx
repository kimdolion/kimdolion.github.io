...
const HeadingsList = ({
  ...
  <ul
    className={`${styles.tableOfContentsList} ${
      isMobile && styles.tableOfContentsMobileList
    }`}
    data-testid={TEST_IDS.tocTestIds.list}
  >
...
const TableOfContentsMobile = ({
  ...
  return (
    <div
      ...
      style={{
        background: isDarkMode ? "#eee" : "#1e1e1e",
        color: isDarkMode ? "black" : "white",
      }}
      className={styles.tableOfContentsMobileList}
    >
      {open ? (
        <HeadingsList
          activeId={activeId}
...
const Headings = ({ headings, activeId, activeHeading }: HeadingsProps) => {
  ...
  const ToCLabel = () => (
    <div
      className={styles.tableOfContentsLabel}
      style={{
        background: isDarkMode ? "#eee" : "#1e1e1e",
...
const getHeadingsData = (headingDepth: number) => {
  ...
  useEffect(() => {
    ...
    const headingElements: HTMLHeadingElement[] = Array.from(
      document.querySelectorAll(HEADING_GROUPINGS[headingDepth]),
    );
    ...
    const mappedElements: HeadingDataProps[] = headingElements.map(
      (heading) => {
...
export const TableOfContents = ({ headingDepth = 0 }: TableOfContentsProps) => {
  ...
  return (
    <nav
      ...
      data-testid={TEST_IDS.tocTestIds.tableOfContents}
