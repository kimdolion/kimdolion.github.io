  ...
  return (
    <div className={styles.projectCard}>
      <div className={styles.projectCardImage}>
        <ImageOverlay
          image={imageAvailable ? projectImage : defaultImage}
          displayRow
        >
          <h3 id={id}>{title}</h3>
      ...
      <div>
        ...
        <p style={{ fontSize: 16, fontWeight: "bold" }}>
          Links to Deployed Site and Github Repos
        </p>
...
const GAProjects = (extraInfo: ProjectsProps) => (
  <div>
    {extraInfo ? (
      <h2 id="projects-bootcamp">During General Assembly</h2>
    ) : (
      <h3 id="projects-bootcamp">During General Assembly</h3>
    )}
    <p>
      These projects were created at the beginning of my Software Engineering
      journey. They are in various states of disrepair but I intend to return to
      them and remake them with newer technology/understandings.
    </p>
    {GENERAL_ASSEMBLY_PROJECTS.map((project) => (
      <div key={project.id}>
...
const TechCard = ({ technologies }: TechCardProps) => {
  return <div className={styles.techList}>{technologies}</div>;
};
...
export const ProjectsSection = ({
  ...
  return (
    <Section>
      ...
      >
        <h2 id="projects-section">Projects and Tech</h2>
      </Link>
      ...
      <div className={styles.techListContainer}>
        <div>
          ...
          {TECHNOLOGIES.frontend.map((technology, index) => (
            <TechCard key={technology + index} technologies={technology} />
          ))}
        </div>
        <div>
        ...
        <div>
          ...
          {TECHNOLOGIES.other.map((technology, index) => (
            <TechCard key={technology + index} technologies={technology} />
          ))}
        </div>
      </div>
      ...
      <div>
        <h3 id="current-projects">
          {extraInfo ? null : "Currently"} Working on:
        </h3>
